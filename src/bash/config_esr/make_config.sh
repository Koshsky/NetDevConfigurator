...
correct_rule() {
    ...
    if [ $3 -eq 1 ]; then
        if sed -i "/<$1>/d" $2 && sed -i "/<@$1>/d" $2; then
            log_message "correct_rule; Успех: Удалены строки с шаблоном '<$1>' и '<@$1>' из файла '$2'."
        else
            log_message "correct_rule; Ошибка: Не удалось удалить строки с шаблоном '<$1>' и '<@$1>' из файла '$2'."
            return 1
        fi
    elif [ $3 -eq 2 ]; then
